...
import { createSuggestions } from "@/modules/preview/libs/contrast-suggestions";
import {
	useColorStore,
	useContrastRelationStore,
} from "@/modules/preview/store/color.store.ts";
import { SuggestionItem } from "./item";
import type { SuggestionProps } from "./suggestion.interface";
...
const Suggestions = ({ type, className, onApply }: SuggestionProps) => {
	...
	function getSuggestions() {
		const output = createSuggestions({
			forColor: isFg ? bg : fg,
			matchingColor: isFg ? fg : bg,
		});

		return output.filter(
			(sug) =>
				parseFloat(sug.contrast) > score.contrast &&
				sug.color !== (isFg ? fg : bg),
		);
	}
