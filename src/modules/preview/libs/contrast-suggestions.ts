...
import { uniqWith } from "@/modules/utils";
import { contrastGuidelines } from "./wcag-text-contrast";
...
function createColorScale(sourceColor: string, opts?: ColorScaleOpts) {
	const defaultOpts = {
		range: [0, 500],
		stepsDistance: 100,
	} satisfies ColorScaleOpts;
	const { range, startInRange, stopInRange, stepsDistance, direction } =
	...
	if (direction === "center") {
		...
		scale = chroma
			.scale([
				chroma(sourceColor).brighten(steps),
				sourceColor,
				chroma(sourceColor).darken(steps),
			])
			.domain(range);
	} else {
		scale = !isDark(sourceColor)
			? chroma
					.scale([sourceColor, chroma(sourceColor).darken(numSteps)])
					.domain(range)
			: chroma
					.scale([sourceColor, chroma(sourceColor).brighten(numSteps)])
					.domain(range);
	}
...
function findAccessibleColors({
	...
	for (const color of matchingColors) {
		...
		if (isAccessible)
			result.push({ id: createHash(), color, contrast: contrast.toFixed(2) });
	}
...
export function createSuggestions({
	...
	if (result.length <= 3) {
		...
		result = [
			...result,
			...findAccessibleColors({ forColor, matchingColors: fallbackScale }),
		];
	}
